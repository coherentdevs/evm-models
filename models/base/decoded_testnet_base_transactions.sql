    ...
    {% if not var('backfill') %} -- if not backfilling
        WITH input_and_transaction_optimism AS (
            SELECT
                ...
                TRY_CAST(hex_to_int(MAX_PRIORITY_FEE) as FLOAT) as MAX_PRIORITY_FEE,
                INPUT,
                LOGS_BLOOM,
                ...
                S,
                True as Status,
                TRY_CAST(hex_to_int(MINT) as FLOAT) as MINT,
                TRY_CAST(IS_SYSTEM_TX as BOOLEAN) as IS_SYSTEM_TX,
                ...
                ACCESS_LIST,
                TRY_CAST(hex_to_int(DEPOSIT_NONCE) as FLOAT) as DEPOSIT_NONCE,
                SUBSTRING(INPUT, 0, 10) AS METHOD_HEADER
            FROM {{ source(var('testnet_base_raw_database'), 'transactions') }}
    ...
    {% else %} -- backfilling
        WITH input_and_transaction_optimism AS (
            SELECT
                ...
                DEPOSIT_NONCE,
                SUBSTRING(INPUT, 0, 10) AS METHOD_HEADER
            FROM {{ source(var('testnet_base_decoded_database'), 'decoded_testnet_base_transactions') }}
